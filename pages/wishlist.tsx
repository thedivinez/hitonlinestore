...
function WishList() {
  const wishlist = useEcommerceState(state => state.wishlist);
  ...
  const totalWishListPrice = useEcommerceState(state => state.totalWishListPrice);
  const removeFromWishlist = useEcommerceState((state) => state.removeFromWishlist);
          ...
          <div className="flex flex-col md:flex-row gap-4">
                ...
                {wishlist.map((product) => (
                  <div className="flex flex-wrap lg:flex-row gap-5  mb-4 items-center" key={product.id}>
                    ...
                    <div className="flex space-x-5 text-xl font-bold items-center">
                      <button className="rounded w-8 h-8 border border-secondary-dark" onClick={() => decreaseQuantity(product, "wishlist")}>-</button>
                      <span>{product.quantity}</span>
                      <button className="rounded w-8 h-8 border border-secondary-dark" onClick={() => increaseQuantity(product, "wishlist")}>+</button>
                    </div>
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <button onClick={() => removeFromWishlist(product)} className="px-4 py-2 inline-block text-primary-light bg-white shadow-sm border border-gray-200 rounded-md hover:bg-gray-100">
                        Remove
                      </button>
                    </div>
                  </div>
            ...
            <aside className="md:w-1/4">
              <article className="border border-gray-200 bg-white shadow-sm rounded mb-5 p-3 lg:p-5">
                ...
                <Link href="/cart">
                  <button className="px-4 py-3 mb-2 inline-block text-md w-full text-center font-medium text-white bg-secondary-light border border-transparent rounded-md hover:bg-secondary-dark"
                  > GO TO CART </button>
                </Link>
                <Link href="/">
